...
public class CustomSwipeRefresh extends ViewGroup {

    public static final String TAG = "CustomSwipeRefresh";
    ...
    private static final int CIRCLE_DIAMETER = 41;
    ...
    private CustomSwipeRefreshDirection mDirection;
    ...
    public CustomSwipeRefresh(Context context, AttributeSet attrs) {
        ...
        final TypedArray a2 = context.obtainStyledAttributes(attrs, R.styleable.CustomSwipeRefresh);
        CustomSwipeRefreshDirection direction
                = CustomSwipeRefreshDirection.getFromInt(a2.getInt(R.styleable.CustomSwipeRefresh_srl_direction, 0));
        if (direction != CustomSwipeRefreshDirection.BOTH) {
            mDirection = direction;
            mBothDirection = false;
        } else {
            mDirection = CustomSwipeRefreshDirection.TOP;
            mBothDirection = true;
        }
        a2.recycle();
     ...
     */
    public void setRefreshing(boolean refreshing, CustomSwipeRefreshDirection direction) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mBothDirection) {
                    if (y > mInitialDownY) {
                        setRawDirection(CustomSwipeRefreshDirection.TOP);
                    } else if (y < mInitialDownY) {
                        setRawDirection(CustomSwipeRefreshDirection.BOTTOM);
                    }
                    if ((mDirection == CustomSwipeRefreshDirection.BOTTOM && canChildScrollDown())
                            || (mDirection == CustomSwipeRefreshDirection.TOP && canChildScrollUp())) {
                        mInitialDownY = y;
                        return false;
                    }
                }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        try {
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (mIsBeingDragged) {
                        ...
                        int targetY;
                        if (mDirection == CustomSwipeRefreshDirection.TOP) {
            ...
            Log.e(TAG, "An exception occurred during CustomSwipeRefresh onTouchEvent " + e.toString());
        }
    ...
    private final Animation mAnimateToCorrectPosition = new Animation() {
        ...
        public void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            int offset = (mFrom + (int) ((endTarget - mFrom) * interpolatedTime)) - mCircleView.getTop();
            setTargetOffsetTopAndBottom(offset  /* requires update */);
        }
    };

    private void moveToStart(float interpolatedTime) {
        setTargetOffsetTopAndBottom((mFrom + (int) ((mOriginalOffsetTop - mFrom) * interpolatedTime)) - mCircleView.getTop()
                /* requires update */);
    }
    ...
    public interface OnRefreshListener {
        void onRefresh(CustomSwipeRefreshDirection direction);
    }

    @SuppressWarnings("unused")
    public CustomSwipeRefreshDirection getDirection() {
        return mBothDirection ? CustomSwipeRefreshDirection.BOTH : mDirection;
    }

    public void setDirection(CustomSwipeRefreshDirection direction) {
        if (direction == CustomSwipeRefreshDirection.BOTH) {
            mBothDirection = true;
        } else {
            mBothDirection = false;
        ...
        switch (mDirection) {
            case BOTTOM:
                mCurrentTargetOffsetTop = -mCircleView.getMeasuredHeight();
                break;
            case TOP:
            default:
                mCurrentTargetOffsetTop = mOriginalOffsetTop = -mCircleView.getMeasuredHeight();
    ...
    // only TOP or Bottom
    private void setRawDirection(CustomSwipeRefreshDirection direction) {
